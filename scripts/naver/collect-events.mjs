#!/usr/bin/env node
// 네이버뉴스 단독 수집 CLI -- 이 소스 하나만 따로 테스트할 때 쓴다.
...
//
// 사용법:
//   node --env-file=.env scripts/naver/collect-events.mjs [YYYY-MM-DD]
//   날짜를 생략하면 오늘(KST) 날짜로 수집한다.
import { writeFile, mkdir } from "node:fs/promises";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { collectNaverCandidates } from "./collect.mjs";
...
async function main() {
  ...
  console.log(`[네이버뉴스 수집] 대상 날짜: ${isoDate} (검수 게이트 미적용, needs_review 원본)`);

  const candidates = await collectNaverCandidates(dateStr);
  const events = candidates.map((c) => c.event);
  console.log(`수집된 후보: ${events.length}건`);
  ...
  console.log(`저장 완료(네이버뉴스 단독, 검수 전 원본): ${path.relative(ROOT_DIR, outFile)}`);
}
